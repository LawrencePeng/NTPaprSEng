...
public final class Helper {
    ...
    public static List<PaperMetricsLink> loadMetricsLinks() {
        ...
        HikariDataSource mysqlDataSource = DataSource.getMysqlDataSource();
        //从第二张数据表中取出已有所有论文相关指标页面链接
        try (Connection connection = mysqlDataSource.getConnection()) {
            try (PreparedStatement preparedStatement = connection.prepareStatement("SELECT URL FROM REF_DATA")) {
    ...
    public static int getRefDataNum() {
        ...
        try (Connection connection = mysqlDataSource.getConnection()) {
            try (PreparedStatement preparedStatement = connection.prepareStatement("SELECT * FROM NT_PAPERS")) {
                try (ResultSet results = preparedStatement.executeQuery()) {
                    while (results.next()) {
                        num++;
                    }
    ...
    public static String getLastUrlForLastTime() {
        ...
        //从第二张数据表中取出已有所有论文相关指标页面链接
        try (Connection connection = mysqlDataSource.getConnection()) {
            try (PreparedStatement preparedStatement = connection.prepareStatement("SELECT URL FROM NT_PAPERS LIMIT 1")) {
                try (ResultSet results = preparedStatement.executeQuery()) {
                    while (results.next()) {
